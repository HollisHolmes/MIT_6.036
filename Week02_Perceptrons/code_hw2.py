...
def perceptron(data, labels, params={}, hook=None):
    ...
    th = np.zeros((d,1))
    th0 = np.zeros((1,1))
    #hyper-parameter that determines how many loops through the data you make
    for iter in range(T):
        #loop through each x,y pair
        for col in range(n):
            x = data[:, col:col+1]
            y = labels[:, col:col+1]
            #check if this pair was inorrectly categorized
            # print(f'x: \n{x}  y: {y}\nth: \n{th}  \nthTx = \n{np.dot(th.T, x)}\n')
...
q1x = np.array([[1, -1, -1],[0, 10, -1]])
q1y = np.array([[-1, 1, 1]])
q2x = np.array([[-4, -2, -1, -2, 1],[2, 1, -1, 2, -2]])
q2y = np.array([[1, 1, -1, -1, -1]])
q5x = np.array([[0,-1],[1,0]])
q5y = np.array([[1, 1]])
perceptron(q5x, q5y, params={}, hook=None)
...
def eval_classifier(learner, data_train, labels_train, data_test, labels_test):
    ...
    class_score = score(data_test, labels_test, th, th0)
    (d, n) = data_test.shape
    return class_score/n
...
def eval_learning_alg(learner, data_gen, n_train, n_test, it):
    ...
    for iter in range(it):
        ...
        learn_score += eval_classifier(learner, data_train, labels_train, data_train, labels_train)

    return learn_score/it
...
def xval_learning_alg(learner, data, labels, k):
    list_of_split_data = np.array_split(data, k, 1)
    list_of_split_labels = np.array_split(labels, k, 1)
    learning_alg_score = 0
    for i in range(k):
        data_train = np.concatenate((list_of_split_data[:i] + list_of_split_data[i+1:]), 1)
        labels_train = np.concatenate((list_of_split_labels[:i] + list_of_split_labels[i+1:]), 1)
        data_test = list_of_split_data[i]
        labels_test = list_of_split_labels[i]
        learning_alg_score += eval_classifier(learner, data_train, labels_train, data_test, labels_test)
    return learning_alg_score/k
...
#     eval_accuracy += eval_learning_alg(perceptron, gen_flipped_lin_separable(pflip=.1), 20, 20, 5)
# print(f'eval accuracy over {trials} trials is: {eval_accuracy/trials}')
