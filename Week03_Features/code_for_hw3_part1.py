...
def positive_nooff(x, th):
    # print(f'pos nooff with x: {x}  theta: {th}  is {y_nooff(x, th)}')
    return np.sign(y_nooff(x, th))
...
def one_hot(x, k):
    out = [1 if i==(x-1) else 0 for i in range(k)]
...
# a = [make_polynomial_feature_fun(order)(features) for order in orders]
# print(a)

def test_with_features(dataFun, order = 2, draw=True, pause=False):
...
def perceptron_nooff(data, labels, params = {}, hook = None):
    T = params.get('T', 100)
    (d, n) = data.shape
    m = 0
    trys = 0
    ...
    for t in range(T):
        for i in range(n):
            x = data[:,i:i+1]
            y = labels[:,i:i+1]
            if y * positive_nooff(x, theta) <= 0.0:
                m += 1
                theta = theta + y * x
                trys += 1
                # if hook: hook((theta, theta_0))
...
data = np.array([[200, 800, 200, 800],
        ...
        [1, 1, 1, 1]])
#1d data
data = np.concatenate((data[0:1, :]*0.001, data[1:, :]), axis = 0)
print(data)
...
margins = []
for col in range(n):
    cur_x = data[:, col:col+1]
    # print('curx : {}'.format(cur_x))
    cur_margin = labels[:, col] * np.dot(theta.T, cur_x)/np.linalg.norm(theta)
    margins.append(cur_margin)
# print(f'margin is {min(margins)}')

#1c
# theta, trys = perceptron_nooff(data, labels, params = {'T': 100}, hook = None)
# print('perceptron took {} trys'.format(trys))
...
# for i in range(6):
#     val = data[0:1, i:i+1]
#     one_hots.append(one_hot_internal(val, 6))
# data = np.concatenate(one_hots, axis = 1)
# data = np.concatenate((data, [[1, 1, 1, 1, 1, 1]]), axis = 0)
# print(data)
# theta, trys = perceptron_nooff(data, labels, params = {'T': 100}, hook = None)
# print(f'theta: {theta}')
#
# print('test at the end')
a = test_with_features(xor_more, order = 3)
print(a)
